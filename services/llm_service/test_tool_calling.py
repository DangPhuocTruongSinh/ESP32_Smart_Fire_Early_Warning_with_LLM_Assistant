"""
Test Suite: Kiểm tra khả năng gọi tool của LLM

Sử dụng trực tiếp tools và cấu hình LLM từ chat_api.py.
Mock toàn bộ MQTT/InfluxDB để chạy mà không cần phần cứng hoặc broker thật.

Cách chạy (từ thư mục gốc project):
    python -m services.llm_service.test_tool_calling
"""

# ==============================================================
# MOCK SETUP - Phải đứng đầu file, TRƯỚC mọi import chat_api
# ==============================================================

import sys
from unittest.mock import MagicMock

# --- Mock paho (chưa cài hoặc tránh kết nối thật) ---
_mock_paho_mqtt_client = MagicMock()
_mock_paho_mqtt_client.MQTT_ERR_SUCCESS = 0
sys.modules.setdefault("paho",            MagicMock())
sys.modules.setdefault("paho.mqtt",       MagicMock())
sys.modules["paho.mqtt.client"] = _mock_paho_mqtt_client

# --- Mock services.ingestion (ngăn mqtt_to_influxdb.py chạy và kết nối) ---
_mock_publish_result = MagicMock()
_mock_publish_result.rc = 0
_mock_publish_result.wait_for_publish.return_value = None

_mock_mqtt_client_instance = MagicMock()
_mock_mqtt_client_instance.publish.return_value = _mock_publish_result

_mock_ingestion = MagicMock()
_mock_ingestion.mqtt_client = _mock_mqtt_client_instance
sys.modules["services.ingestion"]                    = _mock_ingestion
sys.modules["services.ingestion.mqtt_to_influxdb"]   = _mock_ingestion

# --- Thêm create_agent còn thiếu vào langchain.agents ---
try:
    import langchain.agents as _lc_agents
    if not hasattr(_lc_agents, "create_agent"):
        _lc_agents.create_agent = MagicMock(return_value=MagicMock())
except ImportError:
    sys.modules["langchain.agents"] = MagicMock()

# ==============================================================
# IMPORTS (sau khi mock đã sẵn sàng)
# ==============================================================
